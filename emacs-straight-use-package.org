...
Although =package.el= is conveniently built into Emacs, the combination of =use-package= and =straight.el= helps building and configuring reproducable package setups.
...
[[https://github.com/jwiegley/use-package][Use-package]] is a macro to configure and load packages in Emacs configurations that's included in Emacs 29 and up.
...
For example, when using =package.el= without use-package, installing evil-commentary requires running a command (=M-x= =package-install= =<RET>= =evil-commentary= =<RET>=) to install the package.
Then, running =(evil-commentary-mode)= starts the package.
To get the the package to start with Emacs, you need to add it to your init file:
...
Straight.el provides =straight-use-package=, which functions a lot like to =use-package= on first glance.
However, straight.el's version only installs the package, and does not have the configuration abilities of use-package.
Combining straight.el and use-package produces a setup that has the best of both worlds.
...
To configure use-package to /always/ use straight.el without having to pass the =:straight= option, use =use-package= to configure straight.el to turn on =straight-use-package-by-default=[fn:configure-straight]:
...
The resulting =~/.emacs.d/init.el= file installs straight.el and configures it as the package manager for use-package to use:
