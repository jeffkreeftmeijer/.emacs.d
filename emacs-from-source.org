...
- =--program-suffix= :: Suffix the program filename to not overwrite the old version while testing.
...
#+headers: :epilogue ":"
#+begin_src shell
  ./configure --without-x --without-ns --with-native-compilation --with-tree-sitter --program-suffix=-native-comp
#+end_src
