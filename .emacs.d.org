...
* Feed reader
  ...
  By default, elfeed only shows unread entries in the last six months.
  To keep read entries on the list, and see entries that are older than six months, set =elfeed-search-filter= to nil:

  #+begin_src emacs-lisp :tangle init.el
  (setq-default elfeed-search-filter nil)
  #+end_src
