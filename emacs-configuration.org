...
That version is then locked in the ~flake.lock~ file until it's explicitly updated:
...
[[https://github.com/beancount/beancount-mode][Bencount-mode]] requires hooking up the mode manually, so enable it for each file with a =.beancount= extension:
...
[[https://github.com/org-roam/org-roam-ui][Org-roam-ui]] is a graphical frontend for Org-roam, which displays all nodes in a graph for browsing the directory of nodes and discovering possible missing links.
...
[[https://beorgapp.com][Beorg]] is an iOS app that takes Org mode to iOS.
...
#+headers: :exports none
#+headers: :noweb-ref org-agenda-custom
#+begin_src emacs-lisp
